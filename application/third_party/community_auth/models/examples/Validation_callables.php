...
class Validation_callables extends MY_Model {
	...
	public function _check_password_strength( $password )
	{
		...
		if( config_item('max_chars_for_password') > 0 )
			$error .= '<li>Maximo ' . config_item('max_chars_for_password') . ' caracteres</li>';
		...
		if( config_item('min_digits_for_password') > 0 )
		{
			...
			$error .= '<li>' . config_item('min_digits_for_password') . ' numero' . $plural . '</li>';
		}
		...
		if( config_item('min_lowercase_chars_for_password') > 0 )
		{
			...
			$error .= '<li>' . config_item('min_lowercase_chars_for_password') . ' minuscula' . $plural . '</li>';
		}
		...
		if( config_item('min_uppercase_chars_for_password') > 0 )
		{
			...
			$error .= '<li>' . config_item('min_uppercase_chars_for_password') . ' mayuscula' . $plural . '</li>';
		}
		...
		$this->form_validation->set_message(
			...
			'<span class="redfield">Password</span> debe contener:
				<ol>
					' . $error . '
				</ol>
			</span>'
